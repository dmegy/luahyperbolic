...
\item Easy (?) syntax. At least, easier than with \LaTeX macros and pgf.
...
\item No need to compile in "shell-escape" mode.
\item Compile everything in a single pass. No (Leeloo Dallas) multipass. Want to illustrate Euclid's fifth postulate ? Do it right there without leaving your document:

\begin{codeandresult}{Draw and label points}
\begin{hyperbolicdisk}[scale=2.5]
\begin{luacode*}
...
The package also provides lua functions \verb+hyper.beginTikzPicture(options)+ and \verb+hyper.endTikzPicture(export_file)+. 
...
\section{Tilings}
All tilings in this section are draw with \verb+TILING_DEPTH=15+
