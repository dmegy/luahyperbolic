...
\index{symmetryAroundMidpoint}
\begin{codeandresult}
\begin{luacode*}
local A, B = complex.J/2, -complex.I/2
local s = hyper.symmetryAroundMidpoint(A,B)
local points={complex(-.7,.3), complex(.4,.2)}
hyper.tikzBegin()
for k, P in ipairs(points) do
	local sP = s(P)
	hyper.drawSegment(P,sP, "red, dashed")
...
hyper.tikzBegin()
hyper.drawTilingFromTriangle(A, B, C, 12, "teal, draw opacity=.5")
hyper.drawTriangle(A,B,C,"red, very thick")
hyper.tikzEnd()
\end{luacode*}
\end{codeandresult}
...
local PI = math.pi
hyper.tikzBegin()
hyper.drawTilingFromAngles(PI/3, PI/5, PI/7, 12, "teal, draw opacity=.5")
hyper.tikzEnd()
\end{luacode*}
\end{codeandresult}
